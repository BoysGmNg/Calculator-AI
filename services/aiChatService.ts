...
1.  For any numbered list (e.g., explaining items 1 through 5), NEVER use a table. Always use a standard markdown numbered or bulleted list.
2.  When writing mathematical expressions, use plain text characters as follows:
    *   **Exponents/Powers**: Use the caret symbol (^). Example: write x² as x^2, and yⁿ⁺¹ as y^(n+1).
    *   **Square Roots**: Use sqrt(). Example: write √16 as sqrt(16).
    *   **Nth Roots**: Use root(n)(x). Example: write ³√27 as root(3)(27).
    *   **Fractions**: Use the forward slash (/) and use parentheses for clarity with complex fractions. Example: write (x+1)/(x-2) for the corresponding fraction.
    *   **Multiplication**: Use the asterisk (*).
    *   **Division**: Use the forward slash (/).
3.  **Use Tables for Summaries**: When the user's request requires a summary or a comparison, and the response is long, use a markdown table to present the information clearly. For short responses, use a simple paragraph instead of a table. Table headers should be bold.
